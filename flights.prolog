...
route(Departing, Arriving, Visited, DepartureTime, ArrivalTime, Flights, TotalCost) :-
  visited_ports(Departing, Arriving, 0:0, Visited, DepartureTime, ArrivalTime, Flights, TotalCost).

visited_ports(Departing, Arriving, PrevArriveTime,  _, DepartureTime, ArrivalTime, Flights, Cost) :-
  flight(Departing, Arriving, DT, AT, Cost),
  PrevArriveTime @< DT,     %check if the depart time is after the previous arrive time
  DepartureTime = DT,
  ArrivalTime = AT,
  Flights = [Departing, Arriving].

visited_ports(Departing, Arriving, PrevArriveTime, Visited, DepartTime, ArrivalTime, Flights, TotalCost) :-
  flight(Departing, X, DT, AT, InitCost),
  X \= Arriving,            %if x is not destination
  not(member(X, Visited)),  %if x is not one of the airport already visited
  PrevArriveTime @< DT,     %check if the depart time is after the previous arrive time
  visited_ports(X, Arriving, AT, [Departing|Visited], _, ArrivalTime, RestFlights, RestCost),   %recursively call visited_ports to visit next airport
  DepartTime = DT,
  Flights = [Departing|RestFlights],
